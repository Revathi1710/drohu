...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
	...
	if ($door_no==='' || $address_label==='' || $street_address==='' || $area==='' || $receiver_name==='') $errors[] = "All fields are required";

	if (!empty($errors)) {
		echo "<script>window.addEventListener('DOMContentLoaded',()=>showErrorMessage('".implode('. ',$errors)."'));</script>";
	} else {
		...
		if ($stmt->execute()) {
			$_SESSION['selected_address_id'] = $con->insert_id; // set default to the newly added
...
<div class="sticky">
	<div class="foot">
		...
		<button class="btn btn-primary" id="saveBtn" form="addrForm">Save address</button>
	</div>
</div>
...
(function(){
	...
	// Simple required validation toggle for save button
	const form = document.getElementById('addrForm');
	const saveBtn = document.getElementById('saveBtn');
	function validateForm(){
		let ok = true;
		['door_no','pincode','street_address','area','city','state','receiver_name','receiver_number'].forEach(id=>{
			const el = document.getElementById(id);
			if (!el || !el.value.trim()) ok = false;
		});
		if (pincode.value.length!==6 || phone.value.length!==10) ok = false;
		saveBtn.disabled = !ok;
	}
