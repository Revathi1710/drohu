...
<style>
    body{ background:#fafbfc; font-family:Inter,system-ui,Segoe UI,Roboto,Arial,sans-serif; }
    .section-head{ display:flex; align-items:center; justify-content:space-between; padding:14px 16px 8px 16px; }
    .section-head h6{ margin:0; font-weight:700; }
    .product-list{ margin:0 12px 80px 12px; background:#fff; border:1px solid #eef2f7; border-radius:14px; overflow:hidden; }
    .product-row{ display:flex; gap:12px; padding:12px; align-items:center; }
    .product-row + .product-row{ border-top:1px solid #eef2f7; }
    .img-wrap{ position:relative; width:56px; height:56px; flex:0 0 56px; }
    .row-img{ width:56px; height:56px; border-radius:12px; object-fit:cover; background:#f4f6f9; border:1px solid #eef2f7; }
    .size-pill{ position:absolute; left:6px; bottom:-6px; background:#e8f4ff; color:#1b74e4; border:1px solid #cfe6ff; border-radius:999px; padding:2px 8px; font-weight:800; font-size:11px; }
    .row-title{ margin:0; font-size:14px; font-weight:800; line-height:1.2; color:#0b1020; }
    .row-price{ font-weight:900; color:#0b1020; }
    .row-mrp{ color:#6b7280; text-decoration:line-through; font-size:12px; font-weight:700; margin-left:6px; }
    .row-bottom{ display:flex; align-items:center; justify-content:space-between; }
    .stepper{ display:flex; align-items:center; gap:10px; }
    .stepper button{ width:30px; height:30px; border-radius:999px; border:1px solid #dfe4ea; background:#fff; display:flex; align-items:center; justify-content:center; color:#1b74e4; font-weight:900; }
    .stepper .qty{ min-width:22px; text-align:center; font-weight:900; }
    .bottom-nav{ position:fixed; left:0; right:0; bottom:0; background:#fff; border-top:1px solid #eef2f7; display:grid; grid-template-columns:repeat(3,1fr); padding:8px 0; z-index:50; }
 .cart-btn {
    width: 45%;
    align-items: center;
...
;
    ...
    .bottom-nav button{ border:none; background:transparent; color:#6b7280; display:flex; flex-direction:column; align-items:center; gap:4px; font-size:11px; }
    .bottom-nav .active{ color:#1b74e4; }
...
<nav class="bottom-nav-order"  onclick="location.href='cart.php'">
    <button class="cart-btn"><i class="material-icons">add_shopping_cart</i><span>View Cart<br><div id="cart-badge2" class="cart-badge2">0</div>
...
<script>
(function(){
    const badge2 = document.getElementById('cart-badge2');

    function updateCartBadge2(count){
        const n = Number(count||0);
        if (!badge2) return;
        badge2.textContent = n;
        badge2.style.display = n > 0 ? 'flex' : 'none';
    }

    async function refreshCartBadge2(){
        try{
            const res = await fetch('cart_count.php', { credentials:'same-origin' });
            const data = await res.json();
            updateCartBadge2(data.count ?? 0);
        }catch(e){ console.error(e); }
    }

    refreshCartBadge2();
    document.addEventListener('visibilitychange', () => { if (!document.hidden) refreshCartBadge2(); });
    setInterval(refreshCartBadge2, 5000); // every 10s like Zepto

    // Triggered when cart is updated
    window.addEventListener('cart:updated', refreshCartBadge2);
    window.addEventListener('storage', (e) => { if (e.key === 'cartUpdated') refreshCartBadge2(); });
})();
