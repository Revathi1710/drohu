...
$deliveryPersonId = isset($_GET['id']) ? (int)$_GET['id'] : 0;
if ($deliveryPersonId <= 0) {
  http_response_code(400);
  echo '<div style="padding:16px;color:#b91c1c;background:#fee2e2;border:1px solid #fecaca;border-radius:8px;">Invalid delivery person reference.</div>';
  exit;
}

/* Fetch delivery person info for header */
$dp = null;
if ($stmt = $con->prepare('SELECT id, deliveryperson_name, number, email, pincode FROM deliveryPerson WHERE id = ? LIMIT 1')) {
  ...
  $stmt->execute();
  $res = $stmt->get_result();
  $dp = $res ? $res->fetch_assoc() : null;
  $stmt->close();
}
if (!$dp) {
  http_response_code(404);
  echo '<div style="padding:16px;color:#b91c1c;background:#fee2e2;border:1px solid #fecaca;border-radius:8px;">Delivery person not found.</div>';
  exit;
}
...
$whereSql = 'WHERE o.delivery_person_id = ?';
$params = [$deliveryPersonId];
$types  = 'i';

if ($from !== '' && preg_match('/^\d{4}-\d{2}-\d{2}$/', $from)) {
  $whereSql .= ' AND DATE(o.delivered_at) >= ?';
  $params[] = $from;
  $types   .= 's';
}
...
/* Fetch addresses for these orders (to show receiver and location) */
$addressById = [];
...
<head>
  ...
  <title>Delivered Orders · <?php echo h($dp['deliveryperson_name']); ?></title>
  ...
  <style>
    ...
    *{box-sizing:border-box}
    ...
    .hero{
      ...
      background:#fff; padding: clamp(16px,5vw,28px) 18px calc(22px + env(safe-area-inset-top));
      border-radius: var(--radius-xl);
    
      box-shadow: var(--shadow-lg);
      margin-bottom: 12px;
    }
    .container-narrow{ max-width: 1100px; width:100%; margin:0 auto; padding: 12px 0 28px; }
    ...
    .order-badge{ font-size:12px; border-radius:999px; padding:4px 10px; font-weight:700; background:#e8f5e9; color:#2e7d32; }
  ...
  <main class="main-container">
    ...
    <div class="container-narrow">
      <div class="d-flex align-items-center justify-content-between">
        <div>
          <h1 class="brand-title mb-1">Delivered Orders · <?php echo h($dp['deliveryperson_name']); ?></h1>
          <div class="sub">Mobile: <strong><?php echo h($dp['number']); ?></strong> · Pincode: <strong><?php echo h($dp['pincode']); ?></strong></div>
        </div>
        <div>
          <a href="allDeliveryPerson.php" class="btn btn-light btn-sm">Back</a>
        </div>
      </div>
    </div>
  </header>
    <div class="container-narrow">
      <form method="get" class="filter-card">
        <input type="hidden" name="id" value="<?php echo (int)$deliveryPersonId; ?>">
        <div class="row g-2">
          <div class="col-sm-4">
      ...
      <?php if (!$orders): ?>
        <div class="card card-elevated p-4">
          <div class="text-muted">No delivered orders found for the selected filters.</div>
        </div>
      <?php else: ?>
          ...
          <div class="card card-elevated mb-3">
            <div class="card-body">
              <div class="d-flex justify-content-between align-items-start">
                <div>
                  <div class="mb-1">
                    <span class="order-badge">#<?php echo h($oid); ?> · Delivered</span>
                  </div>
                  ...
                  <div class="text-muted small">
                    ...
                    Delivered: <strong><?php echo h($o['delivered_at'] ? date('d M Y, h:i A', strtotime($o['delivered_at'])) : '-'); ?></strong>
                  </div>
                  <?php if ($addr): ?>
                    <div class="addr mt-2">
